...
# -------------------------
class Cfg:
    train_dir         = "./my_faces/train"  # carpeta con subcarpetas por prompt (+ opcional opt_env.txt por imagen)
    out_dir           = "./finetuned"
        ...
        for prompt_folder in sorted(os.listdir(root)):
            ...
            for fn in sorted(os.listdir(pf)):
                ...
                if lo.endswith((".png", ".jpg", ".jpeg", ".webp")):
                    ...
                    cap_path = os.path.splitext(img_path)[0] + "opt_env.txt"
...
if cfg.steps_per_epoch is not None:
    # Queremos EXACTAMENTE steps_per_epoch pasos por época:
    num_samples = cfg.steps_per_epoch * cfg.batch_size
...
global_step = 0
max_steps = cfg.steps_per_epoch if cfg.steps_per_epoch is not None else len(loader)
...
print(f"epoch {epoch+1}: pasos ejecutados = {step_in_epoch}")
