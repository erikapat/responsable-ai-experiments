...
_CLIP_AVAILABLE: bool = True  # se pone False si la carga falla
...
def simple_hist_emb(img: Image.Image, bins: int = 16) -> np.ndarray:
    ...
    return v  # tamaño 48 por defecto
...
def _build_nsfw_pipeline_fast(logger: Optional[logging.Logger] = None):
    """
    Construye el pipeline NSFW con procesador 'fast' y pesos en safetensors si están disponibles.
    Hace fallback a la API simple de pipeline si algo falla.
    """
    model_id = "AdamCodd/vit-base-nsfw-detector"
    try:
        proc = AutoImageProcessor.from_pretrained(model_id, use_fast=True)
        mdl  = AutoModelForImageClassification.from_pretrained(model_id, use_safetensors=True)
        try:
            # versiones nuevas
            pipe = hf_pipeline(
        ...
        if logger: logger.info("[SAFETY] NSFW detector cargado (fast)")
        return pipe
    except Exception as e:
        if logger: logger.warning(f"[SAFETY] NSFW fast falló ({e}) -> intentando pipeline simple")
        try:
            pipe = hf_pipeline("image-classification", model=model_id,
                               device=0 if DEVICE=="cuda" else -1)
            if logger: logger.info("[SAFETY] NSFW detector cargado (simple)")
            return pipe
        except Exception as e2:
            if logger: logger.warning(f"[SAFETY] NSFW no disponible ({e2}) -> usando stub (nsfw=0.0)")
            return _make_nsfw_stub()

def load_safety(caption_needed: bool, logger: logging.Logger):
    global _nsfw, _captioner
    # NSFW classifier
    if _nsfw is None:
        _nsfw = _build_nsfw_pipeline_fast(logger)

    # Captioner (solo si es necesario)
    if caption_needed and _captioner is None:
        try:
            # OJO: BLIP puede venir en .bin → si falla, usamos stub
...
def supports_callback_on_step_end(pipe) -> bool:
    import inspect
    try:
...
def batched_generate(
    ...
    for start in range(0, n, gen_batch_size):
        ...
        try:
            use_on_step_end = supports_callback_on_step_end(pipe)

            if step_progress_every and step_progress_every > 0:
                if use_on_step_end:
                    # Callback tolerante a ambas firmas:
                    # (self, step, timestep, callback_kwargs)  <-- firmas nuevas
                    # (step, timestep, callback_kwargs)        <-- por si cambia
                    def _on_step_end(*args, **kwargs):
                        # Desempaquetar robustamente
                        if len(args) == 4:
                            _, step, timestep, callback_kwargs = args
                        elif len(args) == 3:
                            step, timestep, callback_kwargs = args
                        else:
                            step = kwargs.get("step")
                            timestep = kwargs.get("timestep")
                            callback_kwargs = kwargs.get("callback_kwargs", {})

                        if step is not None and (step_progress_every > 0) and (step % step_progress_every == 0):
                            try:
                                logger.info(f"  [STEP] batch {start + b}/{n} step {step}/{steps} t={timestep}")
                            except Exception:
                                pass
                        # IMPORTANTE: devolver SIEMPRE callback_kwargs
                    ...
                    # Compatibilidad con versiones antiguas (callback/callback_steps)
                    def _cb(i, t, kwargs):
                        if i % step_progress_every == 0:
                            try:
                                ...
                                pass
                        # Debe devolver None o kwargs según versión; por seguridad devolvemos None
                        return None
...
def main():
    ...
    ap.add_argument("--caption-mode", type=str, default="auto", choices=["auto","unsafe","all","none"],
                    help="AUTO: caption borderline; UNSAFE: caption a borderline; ALL: a todos; NONE: sin caption")
